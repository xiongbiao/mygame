...
/**
 * @author 
 * @version Time：2012-8-29 03:34:16
 */
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
	...
	public static final int MAX = Def.KEY_NUM;
	...
	private void init() {
		...
		random = new Random();
		vecKeywords = new Vector<KeyVo>(MAX);
	...
	public boolean feedKeyword(List<KeyVo> keyword) {
...
//			TextView[] txtArr = new TextView[size];
			...
			for (int i = 0; i <  size; i++) {
				String keyword = (vecKeywords.get(i)).getKeyValue();
				/** 随机颜色 */
				...
				txt.setId(-1*(vecKeywords.get(i)).getKeyId());
				
//				txt.setLayoutParams(new LayoutParams(800, 30));
...
//							yMove = 0;
//						}
//						// LogUtil.d("Android_LAB", "break");
//						break inner;
//					}
//				}
//			}
			
			//LogUtil.d("Android_LAB", txt.getText() + " yMove=" + yMove);
//			if (yMove > yItem) {
...
//			layParams.topMargin = 20+i*10;
			txt .setBackgroundColor(232);
	...
	/**
	 * * 根据与中心点的距离由近到远进行冒泡排序。 * * @param endIdx * 起始位置。 * @param txtArr *
	 * 待排序的数组。 *
	 */
	...
	public Vector<KeyVo> getKeywords() {
		return vecKeywords;
	}
