...
public class ExamAdapter extends BaseAdapter {
	...
	private List<ExamVo> mData = new ArrayList<ExamVo>();
	private AsyncImageLoader asyncImageLoader;
	public ExamAdapter(Context context , int resource , List<ExamVo> dataList){
		mContext = context;
		mResource = resource ;
		mData = dataList;
		asyncImageLoader = new AsyncImageLoader();
	}
	public void setData( List<ExamVo> dataList){
		mData = dataList;
	}
	@Override
	public int getCount() {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		}else
		{
		 ...
		 Drawable cachedImage = asyncImageLoader.loadDrawable(mData.get(position).getImgurl(), new ImageCallback() {
	            @Override
...
//			bitmap = ImageUtil.clippingImage(bitmap,holder.imageView.getWidth(),holder.imageView.getHeight(),ImageUtil.CLIP_W_CENTER, ImageUtil.CLIP_H_CENTER);
		...
		holder.title.setText(mData.get(position).getExamname());
