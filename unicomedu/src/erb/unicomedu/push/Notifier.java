...
public class Notifier {
    ...
    public void notifyShow(NotificationVo nv){
    	if(nv==null){
    		return;
    	}
    	 if (isNotificationEnabled()) {
    		 if (isNotificationToastEnabled()) {
//                 Toast.makeText(context, nv.getMsg(), Toast.LENGTH_LONG).show();
             }
             
    		 if(nv.getMsg()!=null&&!nv.getMsg().equals("")){
    			 
              MsgVo mv =  MsgUtil.getStringToObj(nv.getMsg());
                ...
                notification.tickerText = mv.getSummary();
                
                
                
                Intent intent ;
                try {
//               	 String className =  Class.forName(nv.getMsgClassName());
//                	intent = new Intent(context,HomeActivity.class );
                	
                	
                	intent = new Intent(context,MsgInfoActivity.class );
	               
                	Bundle bundle = new Bundle();  
                	 bundle.putSerializable(Def.OBJ,mv);  
                	 bundle.putString("isService", "true");
		               intent.putExtra(Def.OBJ_Bundle, bundle);
//                	switch (Integer.valueOf(nv.getMsgType())) {
//					case 1:
//						bundle = new Bundle(); 
//			            bundle.putSerializable(Def.OBJ,nv);  
//			            intent.putExtra(Def.OBJ_Bundle, bundle);
//						break;
//                    case 2:
//                    	switch (Integer.valueOf(nv.getMsgToType())) {
//	 					   case 1:
//	 						   TeacherVo tv = MsgUtil.getTeacher(nv.getStrData());
//	 						   bundle  = new Bundle();  
...
//					}
                ...
                SharedPreferences settings = context.getSharedPreferences(Def.PREFS_NAME, 0); 
				SharedPreferences.Editor editor = settings.edit(); 
//				editor.(Def.SP_NAV_NAME, tlist);
				String str = settings.getString("msgid", "");
    			if(!str.equals("")){
    				boolean bb = false;
 				   String[] msgList =str.split(",");
 				   for(int i = 0;i<msgList.length;i++){
 					   if(msgList[i].equals(mv.getMsgid())){
 						   bb = true;
 						   break;
 					   }
 				   }
 				   if(!bb){
 					   editor.putString("msgid", mv.getMsgid()+",");
 					  editor.commit();
 					  notification.setLatestEventInfo(context, nv.getTitle(), mv.getSummary(), contentIntent);
    ...
    public void notify(String notificationId, String apiKey, String title,
        ...
        if (isNotificationEnabled()) {
            ...
            Intent intent ;
            try {
            	intent = new Intent(context, Class.forName(title) );
