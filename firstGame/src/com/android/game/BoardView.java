...
import com.android.game.FirstGameActivity.Sound;
import com.android.game.FirstGameActivity.State;
...
public class BoardView extends RelativeLayout {
	...
	/**
	 * Maximum major dimension of the board. The "long" dimension of the board
	 * can't be more than this. This must match up with the dimensions given in
	 * enum Skill below.
	 */
	private static final int BOARD_MAJOR = 9;

	/**
	 * Maximum minor dimension of the board. The "short" dimension of the board
	 * can't be more than this. This must match up with the dimensions given in
	 * enum Skill below.
	 */
	 ...
	 * configuration parameters for that skill level.
	 * 
	 ...
	 * introduce the wrinkle of blind tiles, to make an insane level: Novice:
	 * 4x5 Normal: 6x5 Expert: 6x9 Master: 6x9 wrapped Insane: 6x9 wrapped blind
	 */
	public enum Skill {
		// Name id dims brch wrap blind
		NOVICE(R.string.skill_novice, R.id.skill_novice, 4, 5, 2, false, 9), NORMAL(
				R.string.skill_normal, R.id.skill_normal, 6, 5, 2, false, 9), EXPERT(
				R.string.skill_expert, R.id.skill_expert, 6, 9, 2, false, 9), MASTER(
				R.string.skill_master, R.id.skill_master, 6, 9, 3, true, 9), INSANE(
				R.string.skill_insane, R.id.skill_insane, 6, 9, 3, true, 3);
	 ...
	 */
	public BoardView(FirstGameActivity parent) {
		super(parent);
		parentApp = parent;
	 ...
	 * @param desiredCells
	 *            Number of cells we'd like to have.
	 */
	 ...
	 * @param oldw
	 *            Old width of this view. 0 if we were just added.
	 * @param oldh
	 *            Old height of this view. 0 if we were just added.
	 */
	@Override
		...
		for (int x = 0; x < gridWidth; ++x) {
			for (int y = 0; y < gridHeight; ++y) {
				RelativeLayout.LayoutParams layout = new RelativeLayout.LayoutParams(
						cellWidth, cellHeight);
				if (x > 0)
					layout.addRule(RelativeLayout.RIGHT_OF, y * gridWidth + x);
				else
					layout.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
				if (y > 0)
					layout.addRule(RelativeLayout.BELOW, (y - 1) * gridWidth
							+ x + 1);
				else
	...
	private boolean updateConnections() {
		...
		Log.d(TAG, "updateConnections: " + connections + " connected ("
				+ newConnections + " new)");
	 ...
	 * to connect all the terminals without using all the cable sections. Since
	 * the game intro asks the user to connect all the terminals, which makes
	...
	private static final EnumMap<Cell.Dir, Cell.Dir> contrdirs = new EnumMap<Cell.Dir, Cell.Dir>(
			Cell.Dir.class);
	static {
	...
	private FirstGameActivity parentApp;
